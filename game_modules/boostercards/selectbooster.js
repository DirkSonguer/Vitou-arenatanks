...
var run = function (session, data) {
	...
	var garageItemIds = userObject.userData.boostercards;
	...
	if (garageItem.assemblage != 'booster') {
		logHandler.log('Could not select garage items: Item is not a booster', 3);
		return false;
	}

	// set user data
	if (garageItem.assemblage == 'booster') {
		userObject.userData.activeBoosters.push(data);
	}
	
	storageHandler.set(userObject.id, userObject);

	// send confirmation to client
	var event = '{ "module": "boostercards", "action": "selectedbooster", "data": "' + data + '" }';
